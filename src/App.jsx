...
// ── API CALL ──────────────────────────────────────────────────────────────────
// CORRECTION : fonction unique, sans redéclaration imbriquée
...
const CopyBtn = ({ text }) => {
  ...
  return (
    <button onClick={() => { navigator.clipboard.writeText(text); setOk(true); setTimeout(() => setOk(false), 2000); }}
      className="cz3-btn-sm" style={ok ? { borderColor: "#10B981", color: "#10B981" } : {}}>
      {ok ? "✓" : "Copier"}
    </button>
  );
};
...
const Spin = ({ msg }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 0" }}>
    <div style={{ width: 16, height: 16, border: "2px solid #F5A623", borderTopColor: "transparent", borderRadius: "50%", animation: "cz3Spin .8s linear infinite" }} />
...
const SL = ({ t, opt }) => (
  <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".12em", color: "#B8A898", marginBottom: 8, textTransform: "uppercase" }}>
    {t}{opt && <span style={{ fontWeight: 400, opacity: .6, marginLeft: 6, textTransform: "none" }}>(optionnel)</span>}
  </div>
);
...
function AgentDiagnostic() {
  ...
  const hFile = useCallback(async f => {
    if (!f) return;
    if (f.type !== "application/pdf") { setErr("PDF uniquement."); return; }
    setErr(""); setPdf(f); setB64(await readPdf(f));
  }, []);
  ...
  const gen = async () => {
    ...
    try {
      ...
      uc.push({ type: "text", text: "Génère le rapport L1.1." + (ctx.trim() ? "\n\nContexte : " + ctx : "") });
  ...
  const SBar = ({ s }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ flex: 1, height: 5, background: "rgba(45,10,62,.08)", borderRadius: 3, overflow: "hidden" }}>
        <div style={{ width: `${s * 10}%`, height: "100%", borderRadius: 3, background: s <= 3 ? "#EF4444" : s <= 6 ? "#F59E0B" : "#10B981" }} />
      </div>
      <span style={{ fontSize: 13, fontWeight: 600, minWidth: 18 }}>{s}</span>
    </div>
  );

  if (rep) {
    const c = rep.clientExtrait || {};
    const risk = RS[rep.niveauRisque] || RS["MODÉRÉ"];
    ...
    return (
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid rgba(45,10,62,.08)", flexWrap: "wrap", gap: 12 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".1em", color: "#B8A898", marginBottom: 6, textTransform: "uppercase" }}>{date.toUpperCase()}</div>
        ...
        <div className="cz3-card" style={{ padding: "20px 22px", marginBottom: 12, borderTop: "4px solid #F5A623" }}>
          <SL t="Synthèse" />
          <p style={{ margin: 0, fontSize: 14, lineHeight: 1.8 }}>{rep.synthese}</p>
        </div>
        <div style={{ marginBottom: 12 }}>
          <SL t="7 Piliers" />
          {(rep.pilliers || []).map(p => {
            const s = SS[p.statut] || SS.ORANGE;
            return (
        ...
        <div style={{ marginBottom: 12 }}>
          <SL t="Points critiques" />
          {(rep.pointsCritiques || []).map((p, i) => (
            <div key={i} style={{ padding: "13px 16px", borderRadius: 12, border: "1.5px solid #FCA5A5", background: "#FFF5F5", marginBottom: 7 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#7F1D1D", marginBottom: 4 }}><span style={{ opacity: .4, marginRight: 7 }}>{i + 1}.</span>{p.titre}</div>
              <div style={{ fontSize: 12, color: "#7F1D1D", lineHeight: 1.65, marginBottom: 4 }}>{p.description}</div>
              <div style={{ fontSize: 11, color: "#991B1B", display: "flex", gap: 5 }}><span style={{ opacity: .6 }}>Impact :</span><span>{p.impact}</span></div>
            </div>
          ))}
        </div>
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "repeat(3,1fr)", gap: 10 }}>
            {[{ label: "J+30", a: rep.planAction?.j30, c: "#F5A623" }, { label: "J+60", a: rep.planAction?.j60, c: "#9B8ED4" }, { label: "J+90", a: rep.planAction?.j90, c: "#10B981" }].map(({ label, a, c }) => (
              <div key={label} className="cz3-card" style={{ padding: "13px 15px" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: c, marginBottom: 7, paddingBottom: 6, borderBottom: "1px solid rgba(45,10,62,.06)" }}>{label}</div>
                {(a || []).map((x, i) => <div key={i} style={{ fontSize: 12, color: "#3D2A5C", marginBottom: 5, display: "flex", gap: 6, lineHeight: 1.5 }}><span style={{ color: c, flexShrink: 0, fontWeight: 700 }}>→</span><span>{x}</span></div>)}
              </div>
            ))}
          </div>
        </div>
        <div className="cz3-card" style={{ padding: "16px 20px", borderTop: "4px solid #F5A623", marginBottom: 18 }}>
          <SL t="Recommandation CapZeniths" />
          <p style={{ margin: 0, fontSize: 13, lineHeight: 1.8 }}>{rep.prochainesEtapes}</p>
        </div>
        <div style={{ display: "flex", gap: 10 }} className="no-print">
          <button onClick={reset} className="cz3-btn-sm">← Nouveau</button>
          <button onClick={() => window.print()} className="cz3-btn-sm">🖨 PDF</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 18 }}>
        <SL t="Rapport Cap-Diag (PDF)" />
        {!pdf ? (
          <div onDrop={e => { e.preventDefault(); setDrag(false); hFile(e.dataTransfer.files[0]); }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 16px", borderRadius: 12, border: "1.5px solid #10B981", background: "#ECFDF5" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#065F46", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{pdf.name}</div>
              <div style={{ fontSize: 11, color: "#047857", marginTop: 1 }}>{(pdf.size / 1024).toFixed(0)} Ko</div>
            </div>
      ...
      <div style={{ marginBottom: 20 }}>
        <SL t="Contexte complémentaire" opt />
        <textarea className="cz3-inp" value={ctx} onChange={e => setCtx(e.target.value)} rows={3} placeholder="Notes de l'appel découverte…" style={{ resize: "vertical", lineHeight: 1.6 }} />
      </div>
      ...
      {loading ? (
        <div>
          <div style={{ height: 3, background: "rgba(245,166,35,.15)", borderRadius: 2, overflow: "hidden", marginBottom: 8 }}>
            <div style={{ height: "100%", background: "#F5A623", width: `${pct}%`, transition: "width .6s ease" }} />
          </div>
          <Spin msg={msg} />
        </div>
      ) : (
        <button className="cz3-btn" onClick={gen} disabled={!b64 && !ctx.trim()}>→ Générer le rapport L1.1</button>
      )}
    </div>
  );
}
...
function AgentEditorial() {
  ...
  const gen = async () => {
    ...
    try {
      ...
      const t = TONS.find(x => x.id === form.ton);
      ...
      clearInterval(iv);
      console.error("Erreur éditorial:", e);
      setErr("Erreur. Réessaie.");
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, paddingBottom: 14, borderBottom: "1px solid rgba(45,10,62,.08)" }}>
          <div>
            <span style={{ background: "#F5A623", color: "#2D0A3E", fontSize: 11, fontWeight: 700, padding: "2px 10px", borderRadius: 20 }}>{p?.icon} {p?.label?.toUpperCase()}</span>
            <div className="cz3-serif" style={{ fontSize: m ? 18 : 22, color: "#2D0A3E", marginTop: 5 }}>{art.titre}</div>
          </div>
          <button onClick={reset} className="cz3-btn-sm">← Nouveau</button>
        </div>
            ...
            <div className="cz3-card" style={{ padding: "14px 16px", fontSize: 13, lineHeight: 1.75 }}>
              ...
              {(art.sections || []).map((s, i) => (
                <div key={i} style={{ marginBottom: 9 }}>
                  <div style={{ fontWeight: 600, marginBottom: 3 }}>{s.h2}</div>
                  <div style={{ fontSize: 12, color: "#7C6A8E", lineHeight: 1.7, whiteSpace: "pre-line" }}>{s.contenu}</div>
                </div>
              ))}
              <div style={{ paddingTop: 9, borderTop: "1px solid rgba(45,10,62,.06)" }}>
                <div style={{ fontSize: 12, color: "#7C6A8E", marginBottom: 5, lineHeight: 1.7 }}>{art.conclusion}</div>
                <div style={{ fontSize: 12, fontWeight: 600, color: "#F5A623" }}>{art.cta}</div>
              </div>
            </div>
          </div>
          <div>
            <SL t="5 Déclinaisons" />
            <div className="cz3-tabs-scroll" style={{ marginBottom: 9 }}>
              {DTABS.map(t => <button key={t.id} onClick={() => setTab(t.id)} className={`cz3-tab${tab === t.id ? " sel" : ""}`} style={{ fontSize: 11, padding: "5px 10px" }}>{t.label}</button>)}
            </div>
            <div className="cz3-card" style={{ padding: "13px 15px", minHeight: 220 }}>
              ...
              {tab !== "carousel"
                ? <div style={{ fontSize: 12, color: "#2D1B4E", lineHeight: 1.8, whiteSpace: "pre-line" }}>{decl[tab] || ""}</div>
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 16 }} className="no-print">
        <button onClick={onBack} className="cz3-btn-sm">← Modifier</button>
        <button onClick={() => window.print()} className="cz3-btn" style={{ width: "auto" }}>🖨 Imprimer / PDF</button>
      </div>
      <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 10, padding: "24px", fontFamily: "Georgia,serif", color: "#111" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, paddingBottom: 16, borderBottom: "3px solid #F5A623", flexWrap: "wrap", gap: 10 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#2D0A3E", marginBottom: 2 }}>CapZeniths</div>
            <div style={{ fontSize: 10, color: "#555" }}>Conseil Prévention Défaillance · ogan@capzeniths.com</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 20, fontWeight: 700 }}>{type === "devis" ? "DEVIS" : "FACTURE"}</div>
            <div style={{ fontSize: 11, color: "#555" }}>N° {form.numero} · {form.date}</div>
            {type === "devis" && <div style={{ fontSize: 11, color: "#555" }}>Valide {form.validite} jours</div>}
            {type === "facture" && form.echeance && <div style={{ fontSize: 11, color: "#555" }}>Échéance : {form.echeance}</div>}
          </div>
        </div>
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700 }}>{form.client.nom || "—"}</div>
          {form.client.email && <div style={{ fontSize: 12, color: "#555" }}>{form.client.email}</div>}
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 14 }}>
          <thead>
            <tr style={{ background: "#2D0A3E", color: "#F5A623" }}>
              <th style={{ padding: "7px 10px", textAlign: "left", fontSize: 10, width: 35 }}>QTÉ</th>
              <th style={{ padding: "7px 10px", textAlign: "left", fontSize: 10 }}>DÉSIGNATION</th>
              <th style={{ padding: "7px 10px", textAlign: "right", fontSize: 10, width: 90 }}>P.U. (€)</th>
              <th style={{ padding: "7px 10px", textAlign: "right", fontSize: 10, width: 100 }}>MONTANT</th>
            </tr>
          </thead>
          <tbody>
            {form.prestations.map((p, i) => (
              <tr key={i} style={{ background: i % 2 === 0 ? "#FFFBF0" : "#fff", borderBottom: "1px solid #EDE9F5" }}>
                <td style={{ padding: "8px 10px", fontSize: 12, textAlign: "center" }}>{p.quantite}</td>
                <td style={{ padding: "8px 10px" }}>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{p.titre || "—"}</div>
                  {p.description && <div style={{ fontSize: 11, color: "#666" }}>{p.description}</div>}
                </td>
                <td style={{ padding: "8px 10px", fontSize: 12, textAlign: "right" }}>{fmt(p.prix)}</td>
                <td style={{ padding: "8px 10px", fontSize: 13, fontWeight: 600, textAlign: "right" }}>{fmt(p.quantite * p.prix)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
          <div style={{ minWidth: 200 }}>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid #EDE9F5", fontSize: 12 }}><span>Total HT</span><strong>{fmt(totalHT)} €</strong></div>
            {form.tva && <div style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid #EDE9F5", fontSize: 12 }}><span>TVA ({form.tauxTva}%)</span><strong>{fmt(totalTVA)} €</strong></div>}
            <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", fontSize: 14, fontWeight: 700, color: "#2D0A3E" }}><span>Total {form.tva ? "TTC" : "HT"}</span><span>{fmt(totalTTC)} €</span></div>
          </div>
        </div>
...
function DocForm({ type }) {
  ...
  const [form, setForm] = useState({
    numero: newNum(type === "devis" ? "DEV" : "FAC"),
    date: today(), validite: "30", echeance: "",
    client: { nom: "", email: "", adresse: "" },
    prestations: [{ titre: "", description: "", quantite: 1, prix: 0 }],
    tva: false, tauxTva: 20,
    conditions: type === "devis" ? "Paiement à réception de facture." : "Paiement sous 30 jours.",
    iban: "",
  });
    ...
    <div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr", gap: 8 }}>
          <input className="cz3-inp" value={form.client.nom} onChange={e => sc("nom", e.target.value)} placeholder="Nom / Entreprise" />
          <input className="cz3-inp" value={form.client.email} onChange={e => sc("email", e.target.value)} placeholder="Email" />
        </div>
      </div>
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <SL t="Prestations" />
          <button onClick={addL} className="cz3-btn-sm" style={{ color: "#F5A623", borderColor: "#F5A623" }}>+ Ajouter</button>
        </div>
        {form.prestations.map((p, i) => (
          <div key={i} style={{ padding: "9px 11px", borderRadius: 10, border: "1px solid rgba(45,10,62,.08)", background: "#FAFAF8", marginBottom: 6 }}>
            <div style={{ display: "grid", gridTemplateColumns: m ? "1fr 60px 80px" : "1fr 70px 90px auto", gap: 6, alignItems: "center", marginBottom: 5 }}>
              <input className="cz3-inp" value={p.titre} onChange={e => sl(i, "titre", e.target.value)} placeholder="Titre" />
              <input className="cz3-inp" value={p.quantite} onChange={e => sl(i, "quantite", e.target.value)} placeholder="Qté" />
              <input className="cz3-inp" value={p.prix} onChange={e => sl(i, "prix", e.target.value)} placeholder="Prix HT €" />
              {!m && form.prestations.length > 1 && <button onClick={() => delL(i)} className="cz3-btn-sm" style={{ color: "#991B1B", borderColor: "#FCA5A5" }}>✕</button>}
            </div>
            <input className="cz3-inp" value={p.description} onChange={e => sl(i, "description", e.target.value)} placeholder="Description (optionnel)" />
          </div>
        ))}
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, padding: "7px 0", fontSize: 13 }}>
          <span>HT : <strong>{fmt(tHT)} €</strong></span>
          {form.tva && <span>TVA : <strong>{fmt(tTVA)} €</strong></span>}
          <span style={{ color: "#F5A623", fontWeight: 700 }}>Total : <strong>{fmt(tHT + tTVA)} €</strong></span>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 14 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <input type="checkbox" checked={form.tva} onChange={e => sf("tva", e.target.checked)} />
            <SL t="TVA" />
            {form.tva && <input className="cz3-inp" value={form.tauxTva} onChange={e => sf("tauxTva", e.target.value)} style={{ width: 60 }} />}
          </div>
        </div>
...
function RelanceModule() {
  ...
  if (res) return (
    <div>
      ...
      <div className="cz3-card" style={{ padding: "13px 15px", marginBottom: 9 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}><SL t="Corps" /><CopyBtn text={`Objet: ${res.objet}\n\n${res.corps}`} /></div>
        <div style={{ fontSize: 13, lineHeight: 1.8, whiteSpace: "pre-line" }}>{res.corps}</div>
      </div>
...
function RecapModule() {
  ...
  const [err, setErr] = useState("");
  const sf = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const tI = t => t === "hausse" ? "↗" : t === "baisse" ? "↘" : "→";
  const tC = t => t === "hausse" ? "#10B981" : t === "baisse" ? "#EF4444" : "#F59E0B";

  const gen = async () => {
    if (!form.ca && !form.clients) { setErr("CA ou clients requis."); return; }
    setErr(""); setLoading(true);
    try {
      const r = await callAPI(RCSYS, `Période:${form.mois} ${form.annee}\nCA:${form.ca || "?"}€\nClients:${form.clients || "?"}\nSéances:${form.seances || "?"}\nObjectif:${form.objectif || "?"}\nNotes:${form.notes || "Aucune"}`);
      setRes(r);
    } catch (e) {
  ...
  if (res) return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ fontSize: 15, fontWeight: 600 }}>{form.mois} {form.annee}</div>
        <div style={{ display: "flex", gap: 8 }}>
          <CopyBtn text={`${form.mois} ${form.annee}\n\n${res.synthese}\n\nAlertes:\n${(res.alertes || []).join("\n")}\n\nActions:\n${(res.actions || []).join("\n")}`} />
          <button onClick={() => setRes(null)} className="cz3-btn-sm">← Nouveau</button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 8, marginBottom: 10 }}>
        {(res.indicateurs || []).map((ind, i) => (
          <div key={i} className="cz3-card" style={{ padding: "11px 13px" }}>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".08em", color: "#B8A898", marginBottom: 4, textTransform: "uppercase" }}>{ind.label}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 17, fontWeight: 600 }}>{ind.valeur || "—"}</span>
              <span style={{ fontSize: 14, color: tC(ind.tendance) }}>{tI(ind.tendance)}</span>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: m ? "1fr 1fr" : "1fr 1fr 1fr", gap: 10, marginBottom: 14 }}>
        <div><SL t="Mois" /><select className="cz3-sel" value={form.mois} onChange={e => sf("mois", e.target.value)}>{MOIS.map(mo => <option key={mo}>{mo}</option>)}</select></div>
...
function AgentAdmin() {
  ...
  return (
    <div>
      ...
      {tab === "devis" && <DocForm type="devis" />}
      {tab === "facture" && <DocForm type="facture" />}
...
const MODS = [
  ...
  { id: "editorial", icon: "✍️", label: "Éditorial" },
